"""V34: V27 + nb=48 for n=512 (double-precision panel, 11 steps instead of 16).

Hypothesis: With nb=48 instead of nb=32 for n=512:
- 11 blocking steps instead of 16 (31% fewer cuBLAS calls)
- Each trailing GEMM has K=48 instead of K=32 (better compute density)
- Larger panel fits in B200's 228KB SMEM with double precision
- Expected: 20-30% speedup on (640,512) from fewer launches + better GEMM efficiency

Tier 1 (n=32): CuTe DSL shared-memory Householder QR
Tier 2a (n<=352, batch>=16): FP32 fused panel + CUDA V/T + cuBLAS trailing (nb=32)
Tier 2b (n=512, batch>=16): double panel + CUDA V/T + cuBLAS trailing (nb=48)
...
std::vector<torch::Tensor> dispatch_qr(torch::Tensor A) {
    ...
    if (n <= 512 && batch >= 16) {
        ...
        int nb = is_medium ? 32 : NB;  // nb=48 for n=512, nb=32 for medium
        int threads = 256;
