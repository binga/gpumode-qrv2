...
# ============================================================================
# No-pivot batched LU via cuSOLVER getrf with devIpiv=NULL (BLAS-3, native).
# Used by the n>=2048 Householder-reconstruction path: the reconstruction of
# standard geqrf reflectors from an orthonormal Q reduces to a NO-PIVOT LU of
# (I - Q*S). cuSOLVER's getrf with a null pivot array performs exactly that
# (pivoting disabled), giving the unit-lower L (reflector vectors) + upper U
# (tau on its diagonal). Looped over the small batch (2-8) at these shapes.
# ============================================================================
lu_cuda_src = r"""
#include <torch/extension.h>
...
torch::Tensor lu_nopiv_inplace(torch::Tensor Mt) {
    TORCH_CHECK(Mt.is_cuda() && Mt.dtype() == torch::kFloat32 && Mt.is_contiguous());
    TORCH_CHECK(Mt.dim() == 3 && Mt.size(1) == Mt.size(2));
    const int b = Mt.size(0);
    const int n = Mt.size(1);
    // Use PyTorch's managed cuSOLVER handle, which is already bound to the
    // harness's current stream/context. A privately-created cusolverDnCreate
    // handle issues work the Popcorn anti-cheat flags as "work on another
    // stream" (-> disqualification), even when cusolverDnSetStream is called.
    cusolverDnHandle_t g_lu_handle = at::cuda::getCurrentCUDASolverDnHandle();
    int lwork = 0;
    cusolverDnSgetrf_bufferSize(g_lu_handle, n, n, Mt.data_ptr<float>(), n, &lwork);
    auto work = torch::empty({lwork > 0 ? lwork : 1}, Mt.options());
    auto info = torch::empty({1}, torch::dtype(torch::kInt32).device(Mt.device()));
    for (int i = 0; i < b; ++i) {
        float* Ai = Mt.data_ptr<float>() + (size_t)i * n * n;
        // devIpiv = nullptr  => NO PIVOTING (the reconstruction requires unit-lower L)
        cusolverDnSgetrf(g_lu_handle, n, n, Ai, n,
                         work.data_ptr<float>(), nullptr, info.data_ptr<int>());
    }
    return Mt;
}
"""
...
# below is dlopen'd here at import time.
...
lu_mod = load_inline(
    name="qr_lu_nopiv",
    cpp_sources=[lu_cpp_src],
    cuda_sources=[lu_cuda_src],
    functions=["lu_nopiv_inplace"],
    extra_cuda_cflags=["-O3", "-std=c++17"],
    extra_cflags=["-O3", "-std=c++17"],
...
def _shifted_cholqr(A: torch.Tensor, passes: int, shiftc: float, prec: str):
    ...
    for p in range(passes):
        ...
        # De-batch cuSOLVER potrf / cuBLAS trsm into a single-stream loop over the
        # (small, 2-8) batch: torch.linalg.cholesky on a BATCHED input fans the
        # batch out across a pool of side streams, which Popcorn's anti-cheat
        # rejects as "work on another stream". Per-matrix calls stay on the
        # current stream.
        bsz = G.shape[0]
        R = torch.stack([torch.linalg.cholesky(G[i], upper=True) for i in range(bsz)])
        Q = torch.stack([
            torch.linalg.solve_triangular(R[i], Q[i], upper=True, left=False)
            for i in range(bsz)
        ])
        if Racc is None:
            Racc = R
