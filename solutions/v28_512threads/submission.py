...
std::vector<torch::Tensor> dispatch_qr(torch::Tensor A) {
    ...
    if (n <= 512 && batch >= 16) {
        ...
        for (int j = 0; j < n; j += nb) {
            ...
            // W1 = V^T @ trailing (contiguous inputs → bmm)
            auto trailing_c = trailing.contiguous();
            auto W1_view = W1.slice(1, 0, actual_nb).slice(2, 0, trailing_cols).contiguous();
            at::bmm_out(W1_view, V_view.transpose(1,2), trailing_c);
            // W2 = T^T @ W1
            auto W2_view = W2.slice(1, 0, actual_nb).slice(2, 0, trailing_cols).contiguous();
            at::bmm_out(W2_view, T_view.transpose(1,2), W1_view);
            // trailing -= V @ W2 (fused via baddbmm)
            auto update = at::bmm(V_view, W2_view);
            trailing.sub_(update);
        }
        return {H, tau};
    }
