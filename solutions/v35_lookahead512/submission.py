"""V35: lookahead-pipelined blocked QR for the n=512 family.

Competitive intel (GPU MODE Discord) + our own V32/V33 experiments agree: for
the `(640,512)` family the wall is the *panel factorization and the inter-panel
dependency chain* (panel ~38% of time, latency-bound), NOT the trailing GEMM
(~20%). Tensor cores on the GEMM (V34) therefore didn't move the needle.

This version attacks the dependency chain directly with **lookahead** (classic
LAPACK / producer-consumer): the right-looking blocked update applies reflector
`k` to *all* trailing columns. We split that update by column into:

  * "next-panel columns" -> updated on the MAIN stream, then panel `k+1` is
    factored immediately (latency-bound work), and
  * "the rest"          -> the big WY GEMM, issued on a SIDE stream.

Panel `k+1` (main, latency-bound) thus overlaps with the bulk trailing update of
step `k` (side, throughput-bound), hiding the panel cost behind the GEMM. The WY
update is column-independent so the split is mathematically exact; CUDA events
order the cross-stream reuse of columns. V (Householder vectors) and T are
double-buffered (kept alive in a vector) so the side stream can read step-k's
factors while the main stream builds step-(k+1)'s.

Only n==512 (batch>=16) uses the lookahead path; every other shape is identical
to V31.

Tier 1 (n=32): CuTe DSL shared-memory Householder QR
Tier 2a (n<=352, batch>=16): FP32 fused panel + CUDA V/T + matmul trailing
Tier 2b (n==512, batch>=16): LOOKAHEAD double panel + CUDA V/T, 2 streams  [NEW]
Tier 2c (512<n<=1024, batch>=16): FP32 panel + CUDA V + ATen T + matmul trailing
...
// ----------------------------------------------------------------------------
// Lookahead helpers (stream-aware launchers)
// ----------------------------------------------------------------------------

static void panel_double_on(torch::Tensor H, torch::Tensor tau,
                            int n, int j, int j_end, c10::cuda::CUDAStream s) {
    const int batch = H.size(0), threads = 256, num_warps = threads / 32;
    const int m = n - j;
    size_t smem = ((size_t)m * NB + NB + num_warps) * sizeof(double);
    fused_panel_kernel<<<batch, threads, smem, s.stream()>>>(
        H.data_ptr<float>(), tau.data_ptr<float>(), n, j, j_end);
}

static torch::Tensor build_V_on(torch::Tensor H, int n, int j, int actual_nb,
                                int m_panel, c10::cuda::CUDAStream s) {
    const int batch = H.size(0), threads = 256;
    auto V = torch::empty({batch, m_panel, actual_nb}, H.options());
    int per = m_panel * actual_nb;
    int vb = std::min((per + threads - 1) / threads, 65535);
    build_v_kernel<<<dim3(vb, batch), threads, 0, s.stream()>>>(
        H.data_ptr<float>(), V.data_ptr<float>(), n, j, actual_nb, m_panel, actual_nb);
    return V;
}

static torch::Tensor build_T_on(torch::Tensor V, torch::Tensor tau, int n, int j,
                                int actual_nb, int m_panel, c10::cuda::CUDAStream s) {
    const int batch = V.size(0), threads = 256, num_warps = threads / 32;
    auto T = torch::empty({batch, NB, NB}, V.options());
    size_t t_smem = (NB + num_warps) * sizeof(float);
    build_t_parallel<<<batch, threads, t_smem, s.stream()>>>(
        V.data_ptr<float>(), tau.data_ptr<float>(), T.data_ptr<float>(),
        m_panel, actual_nb, actual_nb, NB, j, n);
    return T;
}

// V31-proven T build via ATen bmm (uses cuBLAS on stream `s`).
static torch::Tensor build_T_aten_on(torch::Tensor V_view, torch::Tensor tau, int n,
                                     int j, int j_end, int actual_nb,
                                     c10::cuda::CUDAStream s) {
    c10::cuda::CUDAStreamGuard guard(s);
    const int batch = V_view.size(0);
    auto T = torch::zeros({batch, NB, NB}, V_view.options());
...
// Apply the block-WY update  trailing -= V (T^T (V^T trailing))  to the column
// range [col_lo, col_hi) of H[j:n, :], using ATen matmul on stream `s`.
static void apply_wy_on(torch::Tensor H, torch::Tensor V, torch::Tensor T,
                        int j, int n, int col_lo, int col_hi,
                        int m_panel, int actual_nb, c10::cuda::CUDAStream s) {
    c10::cuda::CUDAStreamGuard guard(s);
    auto trailing = H.slice(1, j, n).slice(2, col_lo, col_hi);          // (b, m, c)
    auto Vv = V;                                                        // (b, m, nb)
    auto Tv = T.slice(1, 0, actual_nb).slice(2, 0, actual_nb);          // (b, nb, nb)
    auto W1 = at::matmul(Vv.transpose(1, 2), trailing);                 // (b, nb, c)
    auto W2 = at::matmul(Tv.transpose(1, 2), W1);                       // (b, nb, c)
    trailing.sub_(at::matmul(Vv, W2));
}

std::vector<torch::Tensor> lookahead_qr_512(torch::Tensor A) {
    const int batch = A.size(0);
    const int n = A.size(1);
    const int nb = NB;
    const int num_warps = 256 / 32;

    auto H = A.clone();
    auto tau = torch::zeros({batch, n}, A.options());

    // Double panel needs up to n*NB*8 bytes of dynamic smem (n=512 -> 131KB).
    size_t max_smem_d = ((size_t)n * NB + NB + num_warps) * sizeof(double);
    cudaFuncSetAttribute(fused_panel_kernel,
                         cudaFuncAttributeMaxDynamicSharedMemorySize, max_smem_d);

    auto main_s = c10::cuda::getCurrentCUDAStream();
    auto side_s = c10::cuda::getStreamFromPool();
    // ev_A: "next-panel" cuBLAS update done on main; ev_rest: "rest" cuBLAS done
    // on side. PyTorch shares a per-device cuBLAS workspace across streams, so two
    // cuBLAS calls must NOT run concurrently or they corrupt each other. We
    // therefore serialize the GEMMs across streams (A_k -> C_k -> A_{k+1} -> ...)
    // and only overlap the panel factorization (B_k, all custom kernels, no
    // cuBLAS) with the big side GEMM (C_k). That hides the latency-bound panel.
    at::cuda::CUDAEvent ev_A, ev_rest;

    // V/T of consecutive panels coexist (side reads step k while main builds
    // step k+1), so keep every panel's factors alive until the function returns.
    std::vector<torch::Tensor> keepalive;

    const int num_panels = (n + nb - 1) / nb;

    // Factor panel 0 on the main stream.
    panel_double_on(H, tau, n, 0, std::min(nb, n), main_s);
    torch::Tensor V_cur = build_V_on(H, n, 0, std::min(nb, n), n, main_s);
    torch::Tensor T_cur = build_T_on(V_cur, tau, n, 0, std::min(nb, n), n, main_s);
    keepalive.push_back(V_cur);
    keepalive.push_back(T_cur);
    ...
    for (int k = 0; k < num_panels; k++) {
        ...
        if (j_end >= n) break;  // last panel already factored, no trailing

        int next_lo = j_end;
        int next_hi = std::min(j_end + nb, n);
        int rest_lo = next_hi;
        int rest_hi = n;
        bool has_rest = rest_lo < rest_hi;

        // (A) MAIN: apply reflector k to the next panel's columns (small cuBLAS).
        apply_wy_on(H, V_cur, T_cur, j, n, next_lo, next_hi, m, actual_nb, main_s);
        ev_A.record(main_s);

        // (C) SIDE: apply reflector k to the bulk "rest" columns (big cuBLAS).
        // Wait for A_k so the two cuBLAS calls never share the workspace at once;
        // ev_A also guarantees V_cur/T_cur are visible (A_k consumed them).
        if (has_rest) {
            ev_A.block(side_s);
            apply_wy_on(H, V_cur, T_cur, j, n, rest_lo, rest_hi, m, actual_nb, side_s);
            ev_rest.record(side_s);
        }

        // (B) MAIN: factor panel k+1 (all custom kernels) — overlaps side's GEMM.
        int j2 = next_lo, j2_end = next_hi, m2 = n - j2, anb2 = j2_end - j2;
        panel_double_on(H, tau, n, j2, j2_end, main_s);
        torch::Tensor V_nxt = build_V_on(H, n, j2, anb2, m2, main_s);
        torch::Tensor T_nxt = build_T_on(V_nxt, tau, n, j2, anb2, m2, main_s);
        keepalive.push_back(V_nxt);
        keepalive.push_back(T_nxt);

        // Next iter's A_{k+1} reads columns the side stream is still writing, and
        // must not run cuBLAS concurrently with C_k -> main waits for C_k.
        if (has_rest) ev_rest.block(main_s);
