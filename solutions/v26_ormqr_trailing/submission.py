"""V21: V20b + parallel CUDA T-builder + cuBLAS strided trailing (no contiguous copy).
...
std::vector<torch::Tensor> dispatch_qr(torch::Tensor A) {
    ...
    if (n <= 512 && batch >= 16) {
        ...
        for (int j = 0; j < n; j += nb) {
            ...
            // Use ormqr: apply Q^T (from panel reflectors) to trailing columns
            // panel_h contains the Householder vectors in lower triangle
            ...
            // ormqr applies Q^T to trailing: trailing = Q^T @ trailing
            // Q is defined by the Householder vectors in panel_h and scalars in panel_tau
            at::ormqr_out(trailing, panel_h, panel_tau, trailing, /*left=*/true, /*transpose=*/true);

            // Write back trailing (ormqr may have worked on the contiguous copy)
            H.slice(1, j, n).slice(2, j_end, n).copy_(trailing);
        }
        return {H, tau};
    }
