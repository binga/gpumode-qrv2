"""V34: V31 blocked WY + Triton tf32x3 tensor-core trailing update (n>512).

Bold/Blackwell direction: TLX (the Triton fork) isn't importable on the Popcorn
image, but stock Triton 3.7.1 exposes the key lever — `tl.dot(input_precision=
"tf32x3")`, i.e. 3xTF32 emulation that runs the trailing GEMMs on tensor cores
at ~FP32 accuracy. For the FLOP-dominated large shapes the WY trailing update
(trail -= V @ T^T @ V^T @ trail) is pure GEMM, so tensor cores should beat the
plain-FP32 ATen matmul path used by V31.

This version routes the n>512 trailing through a Triton batched tf32x3 GEMM and
keeps every other (well-tuned) path identical to V31.
"""
import torch
...
# ============================================================================
# Triton batched GEMM with tf32x3 (3xTF32) tensor cores.
#   SUB=0: C = A @ B      SUB=1: C = C - A @ B   (in-place on a strided view)
# ============================================================================
@triton.jit
def _bmm_tf32x3(
    A, B, C,
    ...
    a_ptrs = A + pid_b * sa0 + (offs_m[:, None] * sa1 + offs_k[None, :] * sa2)
    b_ptrs = B + pid_b * sb0 + (offs_k[:, None] * sb1 + offs_n[None, :] * sb2)
    ...
    c_ptrs = C + pid_b * sc0 + (offs_m[:, None] * sc1 + offs_n[None, :] * sc2)
    mask = (offs_m[:, None] < M) & (offs_n[None, :] < N)
    if SUB:
        cur = tl.load(c_ptrs, mask=mask, other=0.0)
        tl.store(c_ptrs, cur - acc, mask=mask)
    else:
        tl.store(c_ptrs, acc, mask=mask)


def _tbmm(A, B, C=None, sub=False, bm=64, bn=64, bk=32):
    """Batched A@B (or C-=A@B) via tf32x3. A:(b,M,K) B:(b,K,N)."""
    b, M, K = A.shape
    N = B.shape[2]
    out = C if sub else torch.empty((b, M, N), device=A.device, dtype=torch.float32)
...
// ---- Helpers exposed to Python for the n>512 Triton-trailing path ----
void run_panel_fp32(torch::Tensor H, torch::Tensor tau, int n, int j, int j_end) {
    int batch = H.size(0), threads = 256, num_warps = threads / 32;
    int m_panel = n - j;
    cudaFuncSetAttribute(fused_panel_fp32, cudaFuncAttributeMaxDynamicSharedMemorySize,
                         ((size_t)n * NB + NB + num_warps) * sizeof(float));
...
torch::Tensor run_build_v(torch::Tensor H, int n, int j, int actual_nb, int m_panel) {
    int batch = H.size(0), threads = 256;
    auto V = torch::empty({batch, m_panel, actual_nb}, H.options());
    int per = m_panel * actual_nb;
    int vb = std::min<int>((per + threads - 1) / threads, 65535);
    build_v_kernel<<<dim3(vb, batch), threads>>>(
        H.data_ptr<float>(), V.data_ptr<float>(), n, j, actual_nb, m_panel, actual_nb);
    return V;
}

torch::Tensor run_build_t(torch::Tensor V, torch::Tensor tau, int n, int j,
                          int actual_nb, int m_panel) {
    int batch = V.size(0), threads = 256, num_warps = threads / 32;
...
std::vector<torch::Tensor> dispatch_qr(torch::Tensor A) {
    ...
    if (n <= 512 && batch >= 16) {
        ...
        // n=512 keeps the double panel (batch=640 stress stability). Medium uses
        // the FP32 panel. (n>512 is handled in Python with a Triton trailing.)
...
void run_panel_fp32(torch::Tensor H, torch::Tensor tau, int n, int j, int j_end);
torch::Tensor run_build_v(torch::Tensor H, int n, int j, int actual_nb, int m_panel);
torch::Tensor run_build_t(torch::Tensor V, torch::Tensor tau, int n, int j, int actual_nb, int m_panel);
"""
...
module = load_inline(
    ...
    functions=["dispatch_qr", "run_panel_fp32", "run_build_v", "run_build_t"],
...
def _blocked_triton_trailing(data: torch.Tensor) -> output_t:
    """Blocked WY for 512<n<=1024 with Triton tf32x3 tensor-core trailing update."""
    ...
    for j in range(0, n, nb):
        ...
        j_end = j + actual_nb
        m_panel = n - j
        module.run_panel_fp32(H, tau, n, j, j_end)
        trailing_cols = n - j_end
        if trailing_cols <= 0:
            break
        V = module.run_build_v(H, n, j, actual_nb, m_panel)          # (b, m_panel, actual_nb)
        T = module.run_build_t(V, tau, n, j, actual_nb, m_panel)     # (b, NB, NB)
        T_view = T[:, :actual_nb, :actual_nb]
        trailing = H[:, j:n, j_end:n]                                # strided view (b, m_panel, tc)
        # W1 = V^T @ trailing ; W2 = T^T @ W1 ; trailing -= V @ W2  (all tf32x3)
        W1 = _tbmm(V.transpose(1, 2).contiguous(), trailing)
        W2 = _tbmm(T_view.transpose(1, 2).contiguous(), W1)
        _tbmm(V, W2, C=trailing, sub=True)
    return H, tau
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if cute_qr32 is not None:
        return cute_qr32
    if data.dim() == 3 and 512 < data.size(1) <= 1024 and data.size(0) >= 16:
        return _blocked_triton_trailing(data)
